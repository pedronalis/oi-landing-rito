...
function JourneyCard({ feature, index }: JourneyCardProps) {
  const cardRef = useRef<HTMLDivElement>(null);
  const isInView = useInView(cardRef, { once: true, amount: 0.3 });

  // Left column (0, 2) = Green (Brand), Right column (1, 3) = Purple (Accent)
    ...
    >
      <motion.div
        ...
        className={cn(
          ...
          // Real border instead of background bleed
          isLeft 
      ...
      >
        <div
          className={cn(
            ...
            'bg-[#151515]/80 backdrop-blur-xl', // Lighter dark glass
            'group-hover:shadow-2xl transition-shadow duration-500',
            // Colored shadow on hover
            isLeft 
              ? 'group-hover:shadow-brand-500/10' 
              : 'group-hover:shadow-[#b388ff]/10'
          )}
        >
          {/* No internal colored blobs - strictly neutral */}
          ...
          <div className="relative z-10 flex items-start gap-4 sm:gap-5 p-5 sm:p-6 md:p-8">
            <motion.div
              ...
              className={cn(
                ...
                // Icon container: Color-coded again
                isLeft 
            ...
            <div className="flex-1 min-w-0 pt-1">
              <h3
                className={cn(
                  'text-lg md:text-xl font-bold leading-tight mb-3 transition-colors duration-300',
                  'text-cream-100 group-hover:text-white'
                )}
              >
                {feature.title}
              </h3>
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        >
          <p className="text-brand-400 text-sm md:text-base font-medium mb-6 tracking-wide">
            {description}
          </p>
          <h2 className="max-w-4xl mx-auto text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold leading-tight">
            {Array.isArray(title)
              ? (
                  <>
        ...
        <div className="relative grid md:grid-cols-2 gap-6 lg:gap-8 [--gap:1.5rem] lg:[--gap:2rem]">
          {/* Square U-Pattern Connector (Desktop) */}
